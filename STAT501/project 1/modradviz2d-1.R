...
modradviz2d.fn <- function(x, angles = 2*pi*(0:(ncol(x)-1))/ncol(x), modify = FALSE, cl = NULL, palette = rev(c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")), coord.labels = colnames(x), coord.font = 2, coord.cex = 1.1, class.labels = levels(factor(cl)), class.labels.locations = NULL, ...) {
    ##
...
#    y <- apply(X = x, MARGIN = 2, FUN = function(z)((z - min(z))/(max(z) - min(z))))
    ...
    if (is.null(cl)) {
        ...
        if (is.null(class.labels.locations)) {
            ...
            text(class.labels.locations, col = palette, font = 4, labels = class.labels, ...)
    }
}
...
modradviz2d <- function(x, angles = 2*pi*(0:(ncol(x)-1))/ncol(x), modify = FALSE, cl = NULL, palette = rev(c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")), coord.labels = colnames(x), coord.font = 2, coord.cex = 1.1, class.labels = levels(factor(cl)), class.labels.locations = NULL, opt.axis.order = TRUE, ...) {
    ...
    #print(idx.opt)
    modradviz2d.fn(x = x[,idx.opt], angles = angles, modify = modify, cl = cl, palette = palette, coord.labels = coord.labels[idx.opt], coord.font = coord.font, coord.cex = coord.cex, class.labels = class.labels, class.labels.locations = class.labels.locations, ...)
}
#modradviz2d(x = iris[,1:4], cl = iris[,5], modify = F, coord.labels = c("Sepal Length", "Sepal Width", "Petal Length", "Petal Length"), class.labels.locations = matrix(c(0.4, 0.6, -0.42, -0.1, 0.1, -0.67), ncol = 2, byrow = T))
