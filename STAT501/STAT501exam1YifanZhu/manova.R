...
morel$studentgroup <- C(object = morel$studentgroup, contr = contr.treatment(n = 3,  base = 2))
#
...
newfit1 <- linearHypothesis(model = fit.lm, hypothesis.matrix = C1)
C1 %*% fit.lm$coef
...
# compare with fit.lm1$coef (exactly the same)





peanuts <- read.table(file = "http://www.public.iastate.edu/~maitra/stat501/datasets/peanuts.dat",  col.names=c("location", "variety", "yield", "weight", "seedsize"))
