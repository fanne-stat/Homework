setwd(dir = "c:/Users/fanne/Desktop/")
...
gull <- read.table(file = "./gullsdata.txt", header = T)

oxy <- gull$oxy

bm <- gull$bm

xmat <- matrix(c(rep(1, length(bm)), bm), ncol = 2)

plot(x = bm, y = oxy, xlab = "Body Mass", ylab = "OXY")

j <- regmeanvar(bm, oxy, 6)

plot(log(j$mean),log(sqrt(j$vars)),xlab="Log Group Mean",ylab="Log Group Std. Deviation")

q<-matrix(c(rep(1,length(j$mean)),log(j$mean)),ncol = 2)
linepara <- solve(t(q)%*%q)%*%t(q)%*%log(sqrt(j$vars))
abline(linepara[1], linepara[2])

plot(x = bm, y = log(oxy), xlab = "Body Mass", ylab = "Log OXY")

result <- basicglm(xmat, oxy, 2,5)

plot(x = bm, y = oxy, xlab = "Body Mass", ylab = "OXY")
lines(bm[order(bm)],result$vals$muhat[order(bm)])

plot(result$vals$muhat,result$vals$stdevres, ylab = "Standard Deviance Residual", xlab = "Fitted Value")
abline(0,0)

plot(log(result$vals$muhat),result$vals$stdevres, ylab = "Standard Deviance Residual", xlab = "Log Fitted Value")
abline(0,0)
...
simy <- simbasicglm(c(-5,0.127),xmat,phi = 1,link = 5,random = 3)
result1 <- basicglm(xmat, simy, startb = c(-4, 0.1), 5,3)
plot(x = xmat[,2], y = simy, xlab = "Covariate", ylab = "Response")
lines(xmat[,2], result1$vals$muhat, lty = 2)
lines(xmat[,2], 1-exp(-exp(eta)), lty = 1)

mu <- result1$vals$muhat
Iinverse <- result1$invinf

eta1 <- xmat[,2]*result1$estb[2] + result1$estb[1]

temp <- as.vector(exp(-exp(eta1))*exp(eta1))

D <- matrix(c(temp, temp*xmat[,2]), ncol = 2)

vmu <- diag(D%*%Iinverse%*%t(D))
sdmu <- sqrt(vmu)
...
plot(x = xmat[,2], y = simy, xlab = "Covariate", ylab = "Response")
lines(xmat[,2], result1$vals$muhat, lty = 1)
lines(xmat[,2],lower, lty = 2)
lines(xmat[,2],upper, lty = 2)
