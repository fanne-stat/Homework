setwd(dir = "c:/Users/fanne/Dropbox/Homework/STAT520/STAT520hw4YifanZhu/")
...
nitrate <- read.table(file = "gammagroups.txt", header = T)

xmat1 <- ifelse(nitrate$county == 1, 1, 0)
xmat2 <- ifelse(nitrate$county == 2, 1, 0)
xmat3 <- ifelse(nitrate$county == 3, 1, 0)

xmat <- cbind(xmat1, xmat2, xmat3)

o <- basicglm(xmat,nitrate$y,1,5)

plot(x = c(3,5),xlim = c(3,6), ylim = c(0.5,3.5), xlab = "y", ylab = "county")
lines(x = c(ll[1], ul[1]), y = c(1,1))
lines(x = c(ll[2], ul[2]), y = c(2,2))
lines(x = c(ll[3],ul[3]), y = c(3,3))

alpha <- 3.031
beta <- 0.6988
x <- seq(0,15,0.1)
plot(x = x, y = dgamma(x , shape = alpha, rate = beta ),type = "n", xlab = "y", ylab = "density")
lines(x = x, y = dgamma(x , shape = alpha, rate = beta ))

# ------4.2 -----------
mu <- c(4.243, 4.577, 4.192)

ll1 <- mu - qnorm(0.9833) * semu
ul1 <- mu + qnorm(0.9833) * semu

plot(x = c(2.5,6),xlim = c(2.5,6), ylim = c(0.5,3.5), xlab = "y", ylab = "county")
lines(x = c(ll1[1], ul1[1]), y = c(1,1))
lines(x = c(ll1[2], ul1[2]), y = c(2,2))
lines(x = c(ll1[3],ul1[3]), y = c(3,3))

plot(x = x, y = dgamma(x , shape = 6.603, rate = 1.575 ),type = "n", xlab = "y", ylab = "density")
lines(x = x, y = dgamma(x , shape = 1.778, rate = 0.419 ), lty = 1)
lines(x = x, y = dgamma(x , shape = 3.578, rate = 0.821 ), lty = 2)
lines(x = x, y = dgamma(x , shape = 6.603, rate = 1.575 ), lty = 3)
legend(9.812228, 0.2401705, legend = c("County 1", "County 2", "County 3"), lty = c(1,2,3))
